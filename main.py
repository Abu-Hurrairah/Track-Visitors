...
def checkVisitorInVideo():
    try:
        ...
        time_format = datetime.strptime(time, '%I:%M %p').time()
        ...
        # get_cam_query=f"select id from Camera where name='{camera_name}'";
        #
        # with pyodbc.connect(conn_string) as conn:
        #     with conn.cursor() as cursor:
        #         cursor.execute(get_cam_query)
        #         columns = [column[0] for column in cursor.description]
        #         rows = [dict(zip(columns, row)) for row in cursor.fetchall()]
        #
        # camera_id=rows[0]['id']
        ...
        result = recognizeOneVisitorInVideo(video_filename,visitor_id,camera_name)
    ...
    while True:
        ...
        if previous_visit_history_id == rows[0]['id']:
            ...
            if rows[0]['camera_id'] == source_cam_id and len(newRows)<2:
                ...
                time_difference = int((datetime.strptime(current_time_str, '%H:%M:%S.%f') - datetime.strptime(entry_time_str, '%H:%M:%S.%f')).total_seconds())
                ...
                current_datetime = datetime.strptime(current_time_str, '%H:%M:%S.%f')
                visit_datetime = datetime.strptime(formatted_time_str, '%H:%M:%S.%f')
...
def start_visit_threads():
    try:
        ...
        entry_time = datetime.strptime(startTime, '%I:%M %p').time()
        ...
        for path_index, path in enumerate(paths):
            ...
            for camera_index, each_camera_name in enumerate(path):
                ...
                if is_last_camera:
                    ...
                    response = requests.get(f'{url}/pa/{location_id}', )
                    rows = response.json()
                    if len(rows) != 0:
    ...
    try:
        ...
        while True:
            ...
            for current_visitor_detected in current_visitors_detected_in_frame:
                ...
                if last_visited != camera_name:
                    if is_not_deviated or paths[1][-1] == camera_name:
                        path_history_query = f"insert into VisitPathHistory(visit_id, time, camera_id, is_violated) values ({visit_id}, '{datetime.now()}', {camera_id}, 0)"
                        with pyodbc.connect(conn_string) as conn:
                            with conn.cursor() as cursor:
                                cursor.execute(path_history_query)
                                conn.commit()
                    else:
                        path_history_query = f"insert into VisitPathHistory(visit_id, time, camera_id, is_violated) values ({visit_id}, '{datetime.now()}', {camera_id}, 1)"
                        with pyodbc.connect(conn_string) as conn:
                            with conn.cursor() as cursor:
                                cursor.execute(path_history_query)
                                conn.commit()
